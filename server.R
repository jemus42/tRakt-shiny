...
shinyServer(function(input, output, session) {
  ...
  show <- reactive(label = "show", {
    ...
    withProgress(session, min = 0, max = 5, {
      ...
      setProgress(
        message = "Fetching data from Trakt.tv…",
      ...
      if (file.exists(cachedfile) && (file.info(cachedfile)$mtime - Sys.time()) > -5) {
  ...
  output$show_name <- renderText({
    if (!(isActive())) {
      return("Show Title will appear here soon. Are you excited?")
    }
    show <- show()
    if (is.null(show)) {
  ...
  output$show_links <- renderUI({
    ...
    wiki <- paste0(
      "http://en.wikipedia.org/wiki/Special:Search?search=list of ",
      show$info$title, " episodes&go=Go"
    )

    output <- tags$span(
      bullet, tags$strong(tags$a(href = imdb, "IMDb")),
