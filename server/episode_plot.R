...
observe({
  ...
  plot <- show$episodes %>% ggvis(
    x = as.name(var_x),
    y = as.name(var_y),
    fill = ~season
  )
  plot <- plot %>% layer_points(
    key := ~tooltip, size.hover := 200,
  ...
  plot <- plot %>% add_legend("fill",
    title = "Season", orient = "left",
    properties = legend_props(
      title = list(fontSize = 16),
      labels = list(fontSize = 14)
    )
  )
  plot <- plot %>% add_tooltip(function(epdata) {
    epdata$tooltip
  }, "hover")
